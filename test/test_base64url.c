...
int test_base64url_encodes_correctly() {
	...
	ciron_base64url_encode(b1, 0, chars, &len); /* Can't have empty bytes, so we just use len=0 to mimick */
	EXPECT_BYTE_EQUAL((unsigned char *)"", chars,0);
	ciron_base64url_encode(b1, 1, chars, &len);
	EXPECT_BYTE_EQUAL((unsigned char *)"Zg", chars,2);

	ciron_base64url_encode(b2, 2, chars, &len);
	EXPECT_BYTE_EQUAL((unsigned char *)"Zm8", chars,3);

	ciron_base64url_encode(b3, 3, chars, &len);
	EXPECT_BYTE_EQUAL((unsigned char *)"Zm9v", chars,4);

	ciron_base64url_encode(b4, 4, chars, &len);
	EXPECT_BYTE_EQUAL((unsigned char *)"Zm9vYg", chars,6);

	ciron_base64url_encode(b5, 5, chars, &len);
	EXPECT_BYTE_EQUAL((unsigned char *)"Zm9vYmE", chars,7);

	ciron_base64url_encode(b6, 6, chars, &len);
